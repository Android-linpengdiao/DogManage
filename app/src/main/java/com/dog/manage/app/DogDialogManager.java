...
import com.dog.manage.app.adapter.DogAgeListAdapter;
...
public class DogDialogManager {
    ...
    public void showDogAgeDialog(Activity activity, List<String> list, OnClickListener listener) {
        final AlertDialog dialog = new AlertDialog.Builder(activity, AlertDialog.THEME_HOLO_DARK).create();
        dialog.setCancelable(true);
        dialog.show();
        Window window = dialog.getWindow();
        ...
        window.setContentView(contentView);
        ViewRankDialogBinding binding = DataBindingUtil.bind(contentView);
        DogAgeListAdapter dogAgeListAdapter = new DogAgeListAdapter(activity);
        binding.recyclerView.setLayoutManager(new LinearLayoutManager(activity));
        binding.recyclerView.setAdapter(dogAgeListAdapter);
        dogAgeListAdapter.refreshData(list);
        dogAgeListAdapter.setOnClickListener(new com.base.view.OnClickListener() {
            @Override
            public void onClick(View view, Object object) {
                listener.onClick(view, object);
                dialog.dismiss();
